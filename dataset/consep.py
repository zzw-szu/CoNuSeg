...
classes = {0: "Background", 1: "Others", 2: "Epithelial", 3: "Spindle-shaped"}
...
class CoNSePSegmentation(data.Dataset):
    def __init__(self, root, image_set="train", is_aug=True, transform=None):
        ...
        base_dir = "CoNSeP"
        consep_root = os.path.join(self.root, base_dir)
        splits_dir = os.path.join(consep_root, "splits")

        if not os.path.isdir(consep_root):
            raise RuntimeError(
        ...
        self.images = [
            (os.path.join(consep_root, x[0][1:]), os.path.join(consep_root, x[1][1:]))
            for x in file_names
        ]
...
class CoNSePSegmentationIncremental(data.Dataset):
    def __init__(
        ...
        full_consep = CoNSePSegmentation(root, train, is_aug=True, transform=None)
