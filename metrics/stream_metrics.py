...
class StreamSegMetrics(_StreamMetrics):
    ...
    def get_results(self):
        ...
        dc = 2 * diag / (gt_sum + hist.sum(axis=0) + EPS)
        mean_iu = np.mean(iu[mask])
        mean_dc = np.mean(dc[mask])
        mean_dc_wo_bg = np.mean(dc[mask][1:])
        freq = hist.sum(axis=1) / hist.sum()
        fwavacc = (freq[freq > 0] * iu[freq > 0]).sum()
        cls_iu = dict(
            zip(
                range(self.n_classes), [iu[i] if m else "X" for i, m in enumerate(mask)]
            )
        )
        cls_dc = dict(
            zip(
                range(self.n_classes), [dc[i] if m else "X" for i, m in enumerate(mask)]
            )
        )
        cls_acc = dict(
            zip(
                range(self.n_classes),
                [acc_cls_c[i] if m else "X" for i, m in enumerate(mask)],
            )
        )
        ...
        return {
            ...
            "Mean IoU": mean_iu,
            "Mean Dice": mean_dc,
            "Mean Dice w/o Background": mean_dc_wo_bg,
            "Class IoU": cls_iu,
            "Class Dice": cls_dc,
            "Class Acc": cls_acc,
    ...
    def confusion_matrix_to_fig(self):
        cm = (
            self.confusion_matrix.astype("float")
            / (self.confusion_matrix.sum(axis=1) + 0.000001)[:, np.newaxis]
        )
        fig, ax = plt.subplots()
