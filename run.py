...
from segmentation_module import make_model_v2
...
def main(opts):
    ...
    if opts.step > 0:
        ...
        if not os.path.exists(path):
            if (
                ...
                logger.info(
                    f"[!] WARNING: Checkpoint of old model is {path_FT} and it is NOT copied into {path}"
                )
                path = path_FT
            else:
                try:
                    copy(path_FT, path)
                    logger.info(
                        f"[!] WARNING: Checkpoint of old model is {path_FT} and IT IS copied into {path}"
                    )
                except:
                    path_FT = f"logs/{opts.task}/{opts.task}_FT/{opts.task}-{opts.dataset}_FT//{opts.task}-{opts.dataset}_FT_0.pth"
                    copy(path_FT, path)
                    logger.info(
                        f"[!] WARNINGG: Checkpoint of old model is {path_FT} and IT IS copied into {path}"
                    )
        ...
        if os.path.exists(path):
            ...
            pretrained_dict = {
                k.replace("module.", ""): v
                for k, v in step_checkpoint["model_state"].items()
                if (k.replace("module.", "") in net_dict_old)
            }  # and (
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        pretrained_dict = {
            ...
            if (k.replace("module.", "") in net_dict)
            and (v.shape == net_dict[k.replace("module.", "")].shape)
        }
        net_dict.update(pretrained_dict)
    ...
    if rank == 0 and opts.sample_num > 0:
        sample_ids = np.random.choice(
            len(val_loader), opts.sample_num, replace=True
        )  # sample idxs for visualization
        logger.info(f"The samples id are {sample_ids}")
    else:
        sample_ids = None

    label2color = utils.Label2Color(
        cmap=utils.color_map(opts.dataset)
    ...
    prototypes = torch.zeros(
        [
            sum(tasks.get_per_task_classes(opts.dataset, opts.task, opts.step)),
            opts.feat_dim,
        ]
    )
    ...
    if opts.step > 0 and (opts.loss_pcon > 0):
    ...
    if opts.puce:
        trainer.get_median(train_loader=train_loader, logger=logger)
    ...
    while cur_epoch < opts.epochs and TRAIN:
        ...
        epoch_loss, prototypes, count_features = trainer.train(
            cur_epoch=cur_epoch,
            optim=optimizer,
        ...
        logger.info(
            f"End of Epoch {cur_epoch + 1}/{opts.epochs}, Average Loss={epoch_loss[0] + epoch_loss[1]},"
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            logger.info(
                f"End of Validation {cur_epoch + 1}/{opts.epochs}, Validation Loss={val_loss[0] + val_loss[1]},"
            ...
            if rank == 0:  # save best model at the last iteration
                score = val_score["Mean Dice w/o Background"]
                # best model to build incremental steps
                if score > best_score:
                    best_score = score

                    save_ckpt(
                        f"{logdir_full}/{task_name}_{opts.name}_{opts.step}.pth",
                        model,
                        trainer,
                        optimizer,
                        scheduler,
                    ...
                    logger.info(f"[!] Checkpoint saved. Best score = {best_score}")
            ...
            logger.add_scalar("Val_MeanDice", val_score["Mean Dice"], cur_epoch)
            logger.add_table("Val_Class_Dice", val_score["Class Dice"], cur_epoch)
            ...
            results["V-Dice"] = val_score["Class Dice"]
